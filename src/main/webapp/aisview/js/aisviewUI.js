...
function checkForReload(){
	...
	if (now - reloadPageTime >= idleSince){
...
function recenter(lon, lat, zoom) {
	var center = transformPosition(lon, lat);
...
function setupUI(){
	...
	$("#feedHeader").click(function() {
		if (feedOpen){
			...
			$("#feedHeader").html("Abnormal behaviors - Last " + abnormalFeedLifeTime + " minutes<br /><hr class='tight'>");
	...
	// Set click events on abnormal tool
...
function checkForPanelOverflow(){
	...
	var vdh = 545;				// The height of the vessel details
	if (detailsPanelHeight != null) {
		vdh = detailsPanelHeight;
	}
	var fih = 400;				// The height of the filtering
...
function updateVesselDetails(vesselId){
	...
	$.getJSON(detailsUrl, {
			...
			$("#vd_source_system").html(result.sourceSystem);
			$("#vd_source_region").html(result.sourceRegion);
			$("#vd_source_country").html(result.sourceCountry);
			$("#vd_source_bs").html(result.sourceBs);
